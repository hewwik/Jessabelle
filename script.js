...
BUTTERFLY.prototype = {
	...
	render : function(context){
		...
		for(var i = -1; i <= 1; i += 2){
			...
			gradient.addColorStop(0, 'hsl(348, 100%, 95%)');
			gradient.addColorStop(0.3, 'hsl(348, 100%,  ' + (76 + 10 * rate) +  '%)');
			gradient.addColorStop(0.5, 'hsl(348, 100%, ' + (76 + 20 * rate) +  '%)');
			gradient.addColorStop(1, 'hsl(2348, 100%, ' + (76 + 30 * rate) +  '%)');
			context.lineWidth = 3;
			context.strokeStyle = 'hsl(348, 100%, 84%)';
			context.fillStyle = gradient;
			
			context.save();
			context.scale(0.8 + 0.2 * Math.cos(this.theta + Math.PI / 50), 1);
			context.beginPath();
			context.moveTo(-3, 0);
			context.bezierCurveTo(-20, -5, -30, 10, -15, 20);
			context.bezierCurveTo(-10, 25, -5, 25, -1.5, -2.5);
			context.closePath();
			context.fill();
			context.stroke();
			context.restore();
			...
			context.beginPath();
			context.moveTo(-3, -5);
			context.bezierCurveTo(-12.5, -30, -40, -28, -30, -17);
			context.bezierCurveTo(-27, -5, -60, 2.5, -1.5, 0);
			context.closePath();
			context.fill();
			context.stroke();
			context.restore();
			...
			context.lineWidth = 2;
			context.strokeStyle = 'hsl(351, 19%, 72%)';
			context.beginPath();
			context.moveTo(-2, -10);
		...
		gradient.addColorStop(0, 'hsl(351, 100%, 89%)'); 
		gradient.addColorStop(0.5, 'hsl(351, 100%, 89%)');
		gradient.addColorStop(1, 'hsl(351, 100%, 89%)');
		context.fillStyle = gradient;
		context.beginPath();
		context.moveTo(0, -10);
...
PARTICLE.prototype = {
	...
	render : function(context){
		...
		context.beginPath();
		context.fillStyle =  'hsla(351, 19%, 72%, ' + this.opacity + ')';
